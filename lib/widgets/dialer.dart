...
class Dialer extends StatefulWidget {
  ...
  const Dialer(
      {super.key, required this.options, required this.onItemSelected});
...
class _DialerState extends State<Dialer> {
  late String slectedOption;
  double _rotationAngle = 0.0;

  final constsnts = Get.put(Constants());
  @override
  Widget build(BuildContext context) {
    Map<String, Icon> optionList = widget.options;
    List<String> optionlist = widget.options.keys.toList();

    return GestureDetector(
      onPanUpdate: (details) {
        setState(() {
          _rotationAngle = (_rotationAngle + details.delta.dx / 100) % (2 * pi);
          double degrees = _rotationAngle * 180 / pi;
          // print("00000000 ${widget.options.keys.toList()[0]}");
          if (degrees > 335 || degrees < 15) {
            // print("00000000000000000000000000 1");
            widget.onItemSelected(optionlist[0]);
          }

          if (degrees > 70 && degrees < 115) {
            // print("00000000000000000000000000 2");
            widget.onItemSelected(optionlist[1]);
          }

          if (degrees > 160 && degrees < 200) {
            // print("00000000000000000000000000 3");
            widget.onItemSelected(optionlist[2]);
          }
          if (degrees > 250 && degrees < 280) {
            // print("00000000000000000000000000 4");
            widget.onItemSelected(optionlist[3]);
          }
